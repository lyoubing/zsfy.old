...
namespace NetScape.AnalysisModel
{
    public class HL7MSG
    {
        ...
        public string CreateHL7ADTMsg()
        {
            ...
            builder.AppendLine(string.Format("PV1|1|{4}|{0}^{1}^{2}^^^^^^||||||||||||||||{3}|||||||||||||||||||||||||||||||", PatientLocation, Room, Bed, VisitNumber, PatientType));
            builder.AppendLine(string.Format("ORC|" + cancelFlag + "|{0}|||||1^^^{1}^^Routine|||||^{2}^", OrderNumber, OrderDateTime, OrderPhys));
        ...
        public string CancelHL7ADTMsg()
        {
            ...
            builder.AppendLine(string.Format("OBR|1|||{0}^{1}^^^{2}|||||||||||||||||||||||||||^{3}", HisOrderType, OrderTitle, TestType, TestReason.Replace((char)13, (char)0)));
            msg = string.Format("{0}{1}{2}{3}", MESSAGE_START, builder.ToString(), MESSAGE_END, LAST_CHARACTER);
            msg = msg.Replace("\r\n", "\r");
